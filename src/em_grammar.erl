...
parse(Input) when is_binary(Input) ->
  _ = setup_memo(),
  Result = case 'program'(Input,{{line,1},{column,1}}) of
             {AST, <<>>, _Index} -> AST;
             Any -> Any
           end,
  release_memo(), Result.
...
'program'(Input, Index) ->
  p(Input, Index, 'program', fun(I,D) -> (p_seq([p_zero_or_more(fun 'space'/2), p_one_or_more(fun 'cell'/2), p_zero_or_more(fun 'space'/2)]))(I,D) end, fun(Node, Idx) ->transform('program', Node, Idx) end).
...
'atom'(Input, Index) ->
  p(Input, Index, 'atom', fun(I,D) -> (p_seq([p_zero_or_more(fun 'space'/2), p_choose([fun 'erlsym'/2, fun 'boolean_'/2, fun 'integer'/2, fun 'string'/2, fun 'symbol'/2]), p_zero_or_more(fun 'space'/2)]))(I,D) end, fun(Node, Idx) ->transform('atom', Node, Idx) end).

-spec 'erlsym'(input(), index()) -> parse_result().
'erlsym'(Input, Index) ->
  p(Input, Index, 'erlsym', fun(I,D) -> (p_seq([p_one_or_more(p_seq([p_not(p_string(<<":">>)), p_anything()])), p_string(<<":">>), p_one_or_more(p_seq([p_not(fun 'delimiter'/2), p_anything()]))]))(I,D) end, fun(Node, Idx) ->transform('erlsym', Node, Idx) end).

-spec 'boolean_'(input(), index()) -> parse_result().
'boolean_'(Input, Index) ->
  p(Input, Index, 'boolean_', fun(I,D) -> (p_choose([p_string(<<"#t">>), p_string(<<"#f">>)]))(I,D) end, fun(Node, Idx) ->transform('boolean_', Node, Idx) end).

-spec 'integer'(input(), index()) -> parse_result().
'integer'(Input, Index) ->
  p(Input, Index, 'integer', fun(I,D) -> (p_one_or_more(p_charclass(<<"[0-9]">>)))(I,D) end, fun(Node, Idx) ->transform('integer', Node, Idx) end).

-spec 'string'(input(), index()) -> parse_result().
'string'(Input, Index) ->
  p(Input, Index, 'string', fun(I,D) -> (p_seq([p_string(<<"\"">>), p_zero_or_more(p_choose([p_seq([p_string(<<"\\">>), p_anything()]), p_charclass(<<"[^\"]">>)])), p_string(<<"\"">>)]))(I,D) end, fun(Node, Idx) ->transform('string', Node, Idx) end).

-spec 'symbol'(input(), index()) -> parse_result().
'symbol'(Input, Index) ->
  p(Input, Index, 'symbol', fun(I,D) -> (p_one_or_more(p_seq([p_not(fun 'delimiter'/2), p_anything()])))(I,D) end, fun(Node, Idx) ->transform('symbol', Node, Idx) end).

-spec 'space'(input(), index()) -> parse_result().
'space'(Input, Index) ->
  p(Input, Index, 'space', fun(I,D) -> (p_charclass(<<"[\s\t\n\s\r]">>))(I,D) end, fun(Node, Idx) ->transform('space', Node, Idx) end).

-spec 'paren'(input(), index()) -> parse_result().
'paren'(Input, Index) ->
  p(Input, Index, 'paren', fun(I,D) -> (p_choose([p_string(<<"(">>), p_string(<<")">>)]))(I,D) end, fun(Node, Idx) ->transform('paren', Node, Idx) end).

-spec 'delimiter'(input(), index()) -> parse_result().
'delimiter'(Input, Index) ->
  p(Input, Index, 'delimiter', fun(I,D) -> (p_choose([fun 'paren'/2, fun 'space'/2]))(I,D) end, fun(Node, Idx) ->transform('delimiter', Node, Idx) end).
